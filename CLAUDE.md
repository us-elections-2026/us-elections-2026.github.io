...
### 긴급 (날짜 임박)
- **`actions/checkout@v4` Node 24 대응** — 2026-06-16 강제 전환. `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` env 추가 또는 `checkout@v4.2.x`+ 업그레이드.
- **GA 결선(6/16) 후 데이터 갱신**: `data/senate_races.json`·`data/senate_primaries.json`·`data/model_dashboard.json`·`states/ga.qmd`

### 콘텐츠
- `issues/2026-06-14.qmd` 작성 — ME 6/9 경선 결과·GA 6/16 결선 직전 동향 반영
- `states/nc.qmd`·`states/ak.qmd`·`states/oh.qmd` — 【수집】 슬롯 채우기 (등급·후보 구도·경선 일정)
- `about.qmd` — evergreen 소개 실제 콘텐츠 채우기
- `data/model_dashboard.json` — 6/9·6/16 경선 결과 반영 후 확률 재검토
