...
- `states/{ga,mi,nh,me,nc,tx,oh,ak,ia}.qmd` — 경합주 9곳 State Focus 페이지(`gt_state_detail()` 카드 + 프로즈). 민주 수성 3(GA·MI·NH) + 공화 표적 6(ME·NC·TX·OH·AK·IA). 주 추가/제외는 사람이 결정한다 — **IA는 2026-08-08 편입**(Inside Elections 하향 + Fox 조사 튜렉 우위 + Q2 모금 역전). **탭 구조(2026-09-25 전환)**: 9쪽 모두 `::::: {.panel-tabset}` 안에 **7개 탭**을 이 순서로 둔다 — 주 개요(레이스 카드·그림 + 도입 + `.summary` + 주 개요 본문) · 후보자(`candidate_cards_html` + 경선 절 + `### roll-call / 입법 관심사`) · 선거 제도 · 일일 브리핑 · 최신 여론조사 · 펀드레이징 · 한국 함의. 탭 제목이 목차를 대신하므로 frontmatter에 `toc: false`. 절 번호(1.~7.)는 탭 이름이 순서를 보여주므로 뺐다. 새 내용을 넣을 때는 해당 탭의 `## ` 아래에 `### ` 이하로 넣는다(`## `를 새로 만들면 탭이 하나 늘어난다). 그림 아래 안내 문장은 제거했고 기준일 스탬프만 남겼다(같은 쪽 다른 절로 보내던 링크가 탭에서는 무의미). 주지사 State Focus 8쪽은 **아직 종전 스크롤 구조**.
  **탭별 일일 갱신 블록(2026-09-25)**: 정리본 v1.1의 주별 태그 줄(여론조사·자금·로컬·주 함의)을 `ingest_senate_daily.py`가 `data/state_ledger.json`(주별 장부, 같은 문장은 한 번)으로 옮기고, `state_ledger_md("<주>", cat)`가 탭에 날짜별 목록으로 싣는다 — 주 개요(`state_today_md` 「오늘의 판세」 + `### 로컬 상황 — 일일 기록`) · 최신 여론조사(`### 조사 동향 — 일일 기록`) · 펀드레이징(`### 자금 투입·지출 — 일일 기록`). 손으로 쓴 도입·요약 상자·해설은 그대로 두고 그 아래에 자동 블록이 붙는 구조라, 낡은 수기 문장은 사람이 고쳐야 한다.
  **「일일 브리핑」 절(2026-09-17 신설)**: 카드·요약 상자 아래, `## 1. 주 개요` 앞에 `daily_log_md("<주>")`가 `data/senate_daily_log.json`에서 그날의 「주별 뉴스」 소절을 전재한다(최근 14일, 첫날 펼침·나머지 접힘). `dashboard.qmd`에는 같은 헬퍼의 `"SENATE"` 범위(도입 문단 + 핵심 요약·새 조사 접힘)가 들어간다. 원천은 `_NIS` 상원 일일 브리핑 KR이며 `scripts/ingest_senate_daily.py`가 추출, **`scripts/publish_daily.sh`(Cowork 10:15 KST)가 매일 발행**한다. 이 절은 다체(STYLE.md 예외)이고 프로즈 점검 대상이 아니다.
...
- `scripts/publish_daily.sh --catchup` — 로그 as_of 다음 날부터 오늘까지 정리본/원문이 있는 날짜를 한 번에 적재·발행(2026-09-25). `publish_weekly.sh` 1.9단계가 `NO_COMMIT=1`로 호출해 **10:15 일일 발행이 빠진 날을 일요일 주간 발행이 대신 싣는다**. 여러 날짜를 인자로 줄 수도 있다.
- `scripts/check_site_digest.py` — **정리본↔원문 대조 게이트**(2026-09-21). 10:15 Cowork 작업이 원문을 압축한 `senate_daily/site/<날짜>_site_KR.md`의 모든 숫자가 원문에 있는지·9주 소절 네 요소(판세·이월·주 함의)·조사 표 헤더·분량 상한을 확인. 실패면 `publish_daily.sh`가 중단. 수집(07:00, v2.7)은 제약 없이 넓게, 정리는 발행 때만 — 정리본 규칙의 정본은 `_NIS/…/pipeline_prompts/상원_일일_사이트정리_프롬프트.md`.
- `scripts/extract_daily_polls.py` — **일일 → `senate_polls.csv` 자동 append**(2026-09-17). 정리본(없으면 원문)의 고정 표를 파싱, 같은 조사는 한 행(중복 제거), 날짜·%·URL 미확인은 `senate_polls_candidates.csv`에만. `--from-json`은 LLM 추출 JSON 소급 병합용. 주간 (8)단계는 pending 보완 역할로 축소.
- `scripts/ingest_senate_daily.py` + `scripts/publish_daily.sh` — **일일 발행 경로**(2026-09-17). 상원 일일 브리핑 KR → `data/senate_daily_log.json`(같은 날짜 덮어씀) → 검증·전체 렌더(하드 게이트) → 로그 한 파일만 커밋·push. 그날 파일이 없으면 '발행 없음'으로 정상 종료하고 결손을 채우지 않는다. 주간 발행(`publish_weekly.sh`)과는 독립이며 둘 다 계속 돈다.
