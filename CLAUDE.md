...
- `scripts/extract_daily_polls.py` — **일일 → `senate_polls.csv` 자동 append**(2026-09-17). 상원 일일 프롬프트 v2.8의 고정 표를 파싱, 같은 조사는 한 행(중복 제거), 날짜·%·URL 미확인은 `senate_polls_candidates.csv`에만. `--from-json`은 LLM 추출 JSON 소급 병합용. 주간 (8)단계는 pending 보완 역할로 축소.
