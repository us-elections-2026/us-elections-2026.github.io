...
라이브로 가는 경로가 둘이고, **검수 수준이 다르다.**

**경로 A — 브랜치 + PR (기본).** 사람·에이전트가 손으로 데이터·콘텐츠를 고치는 변경은 이 경로로 낸다. PR에서 Codex와 사람이 diff를 읽고, 병합해야 배포된다.

**경로 B — 주간 자동 발행 (예외, `main` 직커밋).** `scripts/publish_weekly.sh`와 Mac mini 자동 수집은 직커밋을 유지한다. **이 스크립트들은 PR 강제를 위해 고치지 않는다.**

⚠️ **경로 B는 판단 검토를 거치지 않는다 — 알려진 위험이며, 감수하기로 한 것이다.**

- **있는 것**: `validate_data.R`(스키마·파싱·필수 필드·범위)과 `quarto render`. 둘 다 하드 게이트지만 **기계 검증**이다.
- **없는 것**: diff를 읽는 다른 눈. PR이 없으므로 **Codex는 이 경로를 아예 보지 못한다**(Codex는 PR에만 붙는다).
- **그런데** `git add data/ issues/ states/ ./*.qmd`(publish_weekly.sh §5)는 자동 산출물뿐 아니라 **Cowork가 손으로 정규화한 수치와 프로즈까지 함께 커밋한다.** 2026-08-12 전수 대조에서 확인된 사고 5건(2018 벤치마크 혼입 · 출처 없는 파생값 · 전사 슬립 · 자기모순 수치 · 집계↔개별조사 층위 혼동)이 **전부 이 경로로 들어와 약 두 달간 라이브에 있었다.** 다섯 건 모두 스키마상 정상이라 기계 검증을 통과한다.

**경로 B의 보완 — 근거 기록을 함께 커밋한다.** 리뷰어를 대신하지는 못하고, **사후 추적을 가능하게 한다.** 그 주에 손으로 바꾼 `data/` 값이 있으면 `data/weekly_provenance/<YYYY-MM-DD>.md`에 근거를 남긴다. `publish_weekly.sh`가 `data/`를 통째로 스테이징하므로 **스크립트 수정 없이** 주간 발행 커밋에 함께 실린다. 형식은 `data/weekly_provenance/_TEMPLATE.md`. 자동 산출물은 적지 않으며, 손으로 바꾼 값이 없으면 파일을 만들지 않는다.
...
1. **출처 정합**: 새 수치에 원출처(기관·기준일)가 붙어 있는가. 이차보도(뉴스위크·야후 등)만 근거인 수치는 지적하라.
   **`source_url` 요구는 그 필드가 정의된 파일에만 적용하라** — `approval_polls.csv`·`generic_polls.csv`(+`_supplement`)·`senate_polls.csv`·`governor_polls.csv`·`korea_watch.csv`·`pollster_series.json`. 나머지 수동 JSON은 **스키마에 `source_url`이 없고** 각자의 방식으로 출처를 보존한다(`approval.json`·`generic_ballot.json`은 `source`+기준일, `senate_races.json`은 `poll_source`). 이들에 `source_url`을 요구하면 정상 갱신마다 오탐이 나고 시계열 스키마에 임의 필드를 넣게 된다 — 기관·기준일과 PR의 근거 기록으로 검증하라. 어느 쪽이든 값을 넣을 수 없으면 `【수집】`.
...
4. **시간 필드의 방향**: 두 종류를 구분하라.
   - **미래가 정상인 것 — 일정(日程)뿐이다**: 경선일, 투표·등록·우편신청·우편반환 마감일(10~11월). 이걸 "미래 날짜"라고 지적하면 오탐이다.
   - **과거여야 하는 것 — 관측 시점 전부**: `as_of`·`*_as_of`·`row_as_of`, **조사 기간**(`approval_polls.csv`·`generic_polls.csv`의 `date_start`/`date_end`, `senate_polls.csv`·`governor_polls.csv`의 `start_date`/`end_date`), 스냅샷 날짜(`data/history/<날짜>/`). **미래로 찍힌 조사 종료일은 현재 CI가 잡지 못한다** — `validate_data.R`의 CSV 검증은 필수 컬럼과 산술만 본다.

   반대로 **오래된 `as_of`가 곧 오류도 아니다** — 의도적으로 갱신을 보류한 행이 있다(`approval.json`의 USPollingData `row_as_of: 2026-06-12`, note에 "신규 반영 대기"). 신선도·미래날짜를 근거로 지적하기 전에 그 필드가 어느 쪽인지 먼저 확인하라.
...
6. **정정 이력 대조**: 과거에 정정된 것으로 기록된 값들이다 — 2022 투표율은 VEP 46.1%(45.1% 아님), 재획정 순효과 "R+10" 무출처(Cook R+12 또는 Morris ~R+8로 병기), GA-14 특별선거 마진 축소는 기준 병기(그린 대비 ~18p/트럼프 대비 ~25p), 1938년 하원은 APP 기준 −72.
   **단 이 네 값 자체는 1차 자료로 독립 검증되지 않았다** — 리포 안에 해당 기재가 존재하는 것만 확인됐다. **재발 감시용 참고 목록으로 쓰되 불변 상수로 강제하지 말라.** 더 나은 원출처를 근거로 이 값을 바로잡는 PR은 정당하며, 그런 PR에는 여기 적힌 값을 들이대지 말고 **원출처를 요구하라**. 값이 1차 자료와 대조돼 근거 기록에 연결되면 그때 확정 규칙으로 승격한다.
