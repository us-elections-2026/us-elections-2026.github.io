...
- **동시 발행 주의 — 쓰기 직전 git을 다시 확인한다.** 이 레포는 두 머신이 Dropbox로 같은 작업 사본을 공유하고, 그 위에서 **Cowork 예약 작업**이 주간 발행(issue 작성 → data 갱신 → commit → push → 배포)까지 자동으로 수행한다. 세션 초반의 상태 확인은 몇 시간이면 낡으므로 `issues/`·`data/`에 **쓰기 직전 `git log --oneline -3` + `git status`를 재확인**하고, 실행 여부는 파일 존재가 아니라 **커밋·Actions 이력**으로 판정한다. 겹치면 **발행본이 정본**. 미커밋 편집을 오래 남기지 말고(`publish_weekly.sh §5`가 디렉터리째 `git add`한다), **작업 사본을 `main`이 아닌 브랜치에 두고 나오지 말 것**(발행 스크립트가 `main`이 아니면 중단한다). 공유 작업 사본의 다른 함정(충돌 사본·머신 종속 진단)은 `CLAUDE.md`의 「금지·주의」 참조.
